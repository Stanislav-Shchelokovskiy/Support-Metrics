...
def get_baseline_aligned_mode_query(
    ...
    return f"""(SELECT  {TicketsWithIterationsMeta.user_id},
...
INNER JOIN (
    ...
    WHERE   {BaselineAlignedCustomersGroupsMeta.assignment_date} BETWEEN '{kwargs['range_start']}' AND '{kwargs['range_end']}'
            {filter_generator.generate_customer_groups_filter(
                        params=kwargs['customers_groups'],
                        col=BaselineAlignedCustomersGroupsMeta.id
                    )}
